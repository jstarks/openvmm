...
impl IntoPipeline for CheckinGatesCli {
    fn into_pipeline(self, backend_hint: PipelineBackendHint) -> anyhow::Result<Pipeline> {
        ...
        // Container image artifact handles (per-arch, collected during build job setup)
        struct ContainerImageArtifacts {
            use_nextest_archive: Option<UseTypedArtifact<flowey_lib_hvlite::build_nextest_vmm_tests::NextestVmmTestsArchive>>,
            ...
            use_guest_test_uefi: Option<UseTypedArtifact<flowey_lib_hvlite::build_guest_test_uefi::GuestTestUefiOutput>>,
            use_entrypoint: Option<UseTypedArtifact<flowey_lib_hvlite::build_vmm_test_container_entrypoint::VmmTestContainerEntrypointOutput>>,
        }
        let mut container_image_x64 = ContainerImageArtifacts {
            use_nextest_archive: None,
            use_openvmm: None,
        ...
        let mut container_image_aarch64 = ContainerImageArtifacts {
            use_nextest_archive: None,
            use_openvmm: None,
            use_openvmm_vhost: None,
            use_pipette_linux_musl: None,
        ...
        for arch in [CommonArch::Aarch64, CommonArch::X86_64] {
            ...
            // Container entrypoint binary + nextest archive for container image
            let (pub_container_nextest_archive, pub_entrypoint) = {
                let (pub_archive, use_archive) = pipeline
                    .new_typed_artifact(format!("{arch_tag}-linux-container-vmm-tests-archive"));
                let (pub_entrypoint, use_entrypoint) = pipeline
                    .new_typed_artifact(format!("{arch_tag}-linux-vmm-test-container-entrypoint"));
                let ci = match arch {
                    CommonArch::X86_64 => &mut container_image_x64,
                    CommonArch::Aarch64 => &mut container_image_aarch64,
                };
                ci.use_nextest_archive = Some(use_archive);
                ci.use_entrypoint = Some(use_entrypoint);
                (pub_archive, pub_entrypoint)
            };
            ...
            ci.use_openvmm = Some(use_openvmm.clone());
            ci.use_openvmm_vhost = Some(use_openvmm_vhost.clone());
            ci.use_tmks = Some(use_tmks.clone());
            ci.use_vmgstool = Some(use_vmgstool.clone());
            ci.use_guest_test_uefi = Some(use_guest_test_uefi.clone());
        ...
        for arch in [CommonArch::Aarch64, CommonArch::X86_64] {
            ...
            // skim off interesting artifacts required by the VMM tests job
            {
                ...
                ci.use_pipette_linux_musl = Some(use_pipette_linux_musl.clone());
                ci.use_tmk_vmm = Some(use_tmk_vmm.clone());
            }
            match arch {
                CommonArch::X86_64 => {
