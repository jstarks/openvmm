...
use crate::build_hosting_vm::HostingVmOutput;
...
/// Configuration for running tests inside a hosting VM (e.g., QEMU TCG).
#[derive(Serialize, Deserialize)]
pub struct HostingVmParams {
    /// Pre-built hosting_vm binary (runs on the CI host, not inside the VM).
    pub hosting_vm: ReadVar<HostingVmOutput>,
    /// TOML profile content for the hosting VM emulator configuration.
...
flowey_request! {
    pub struct Params {
        ...
        /// If set, run tests inside a hosting VM instead of directly on the host.
        pub hosting_vm: Option<HostingVmParams>,
...
impl SimpleFlowNode for Node {
    ...
    fn process_request(request: Self::Request, ctx: &mut NodeCtx<'_>) -> anyhow::Result<()> {
        ...
        let results = if let Some(hosting_vm_params) = hosting_vm {
            let HostingVmParams {
                hosting_vm: hosting_vm_bin,
                profile_content,
            } = hosting_vm_params;
            ...
            let (profile_path, archive_name) = {
                ...
                let prep = ctx.emit_rust_stepv("prepare hosting VM share directory", |ctx| {
                    ...
                    move |rt| {
                        ...
                        // Write profile
                        let profile_path = test_content_dir.join(".hosting-vm-profile.toml");
            ...
            ctx.reqv(|v| crate::run_in_hosting_vm::Request {
                hosting_vm_bin,
                profile_path,
                kernel,
                initrd,
