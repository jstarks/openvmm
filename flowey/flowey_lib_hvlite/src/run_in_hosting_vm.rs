...
//! Run a nextest archive inside a hosting VM (e.g., QEMU TCG).
//!
//! This node encapsulates the logic for invoking the `hosting_vm` binary
...
flowey_request! {
    pub struct Request {
        /// Path to the hosting_vm binary.
        pub hosting_vm_bin: ReadVar<PathBuf>,
        /// Path to the hosting VM profile TOML file.
...
impl SimpleFlowNode for Node {
    ...
    fn process_request(request: Self::Request, ctx: &mut NodeCtx<'_>) -> anyhow::Result<()> {
        ...
        ctx.emit_rust_step("run tests in hosting VM", |ctx| {
            let hosting_vm_bin = hosting_vm_bin.claim(ctx);
            ...
            move |rt| {
                let hosting_vm_bin = rt.read(hosting_vm_bin);
                ...
                let status = cmd.status().context("failed to launch hosting VM")?;
