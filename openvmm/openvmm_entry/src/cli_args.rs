...
Examples:
    ...
    /// Boot UEFI without exposing hypervisor (HV#1) enlightenments, setting the
    /// firmware's `hv_disabled` flag. Required to boot UEFI without hypervisor
    /// support (e.g. aarch64 KVM). Requires `--no-vmbus` since VMBus depends on
    /// the hypervisor.
