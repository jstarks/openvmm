...
impl QueueSetup {
    ...
    pub fn try_activate(&mut self) -> Option<(QueueResources, u32)> {
        ...
        let notify = self.call.take().unwrap_or_else(Interrupt::null);
...
mod tests {
    ...
    #[test]
    fn try_activate_consumes_kick_and_call() {
        let mut q = QueueSetup::new();
        q.set_num(64);
        q.set_addr(0x1000, 0x2000, 0x3000);
        q.set_kick(Event::new());
        ...
        assert!(q.try_activate().is_some());
        // Second activation fails because kick was consumed.
        assert!(q.try_activate().is_none());
    }
}
