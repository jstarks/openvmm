    ...
    pub(crate) fn handle_dhcp(&mut self, payload: &[u8]) -> Result<(), DropReason> {
        ...
        match dhcp_req.message_type {
            DhcpMessageType::Discover => {
                your_ip = Some(self.inner.common.params.client_ip);
                message_type = DhcpMessageType::Offer;
            }
            DhcpMessageType::Request => {
                your_ip = match dhcp_req.requested_ip {
                    Some(addr) if addr == self.inner.common.params.client_ip => Some(addr),
                    None => Some(self.inner.common.params.client_ip),
                    Some(_) => None,
                };
        ...
        let dns_servers = if self.inner.common.params.nameservers.is_empty() {
            None
        } else {
        ...
        let resp_dhcp = if let Some(your_ip) = your_ip {
            DhcpRepr {
                ...
                your_ip,
                server_ip: self.inner.common.params.gateway_ip,
                router: Some(self.inner.common.params.gateway_ip),
                subnet_mask: Some(self.inner.common.params.net_mask),
                relay_agent_ip: Ipv4Address::UNSPECIFIED,
                broadcast: false,
                requested_ip: None,
                client_identifier: None,
                server_identifier: Some(self.inner.common.params.gateway_ip),
        ...
        let resp_ipv4 = Ipv4Repr {
            src_addr: self.inner.common.params.gateway_ip,
