...
/// Convert a [`vhdx::VhdxError`] to a [`DiskError`].
fn vhdx_to_disk_error(e: vhdx::VhdxError) -> DiskError {
    match e {
        vhdx::VhdxError::Io(io_err) => DiskError::Io(io_err),
        vhdx::VhdxError::ReadOnly => DiskError::ReadOnly,
        other => DiskError::Io(std::io::Error::new(std::io::ErrorKind::Other, other)),
    }
}
        ...
        for range in &ranges {
            match *range {
                ReadRange::Data {
                    ...
                    // Read from the VHDX file into a temporary buffer.
                    let mut data = vec![0u8; length as usize];
                    self.file
                        .read_at(file_offset, &mut data)
                        .await
        ...
        for range in &ranges {
            match *range {
                WriteRange::Data {
                    ...
                    // Read data from the request buffers.
                    let mut data = vec![0u8; length as usize];
                    buffers
                        .subrange(buf_offset, length as usize)
                        .reader()
                    ...
                    // Write to the VHDX file at the resolved offset.
                    self.file
                        .write_at(file_offset, &data)
                        .await
                        .map_err(DiskError::Io)?;
                }
                WriteRange::Zero {
...
mod tests {
    ...
    async fn create_and_open_layer(path: &std::path::Path, driver: &DefaultDriver) -> VhdxLayer {
        ...
        vhdx::create::create(&bf, &mut params).await.unwrap();
