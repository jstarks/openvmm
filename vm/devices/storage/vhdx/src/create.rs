...
pub async fn create(
    ...
    if params.logical_sector_size != 512 && params.logical_sector_size != 4096 {
        return Err(InvalidFormatReason::InvalidLogicalSectorSize.into());
    }
    ...
    if params.physical_sector_size != 512 && params.physical_sector_size != 4096 {
        return Err(InvalidFormatReason::InvalidPhysicalSectorSize.into());
    }

    if params.disk_size == 0 {
        return Err(InvalidFormatReason::DiskSizeZero.into());
    }
    if !params
        .disk_size
        .is_multiple_of(params.logical_sector_size as u64)
    {
        return Err(InvalidFormatReason::DiskSizeNotAligned.into());
    }
    if params.disk_size > format::MAXIMUM_DISK_SIZE {
        return Err(InvalidFormatReason::DiskSizeTooLarge.into());
    }
    ...
    if !(params.block_size as u64).is_multiple_of(format::REGION_ALIGNMENT) {
        return Err(InvalidFormatReason::BlockSizeNotAligned.into());
    }
    if params.block_size as u64 > format::MAXIMUM_BLOCK_SIZE {
        return Err(InvalidFormatReason::BlockSizeTooLarge.into());
    }

    if params.block_alignment != 0 && !params.block_alignment.is_power_of_two() {
        return Err(InvalidFormatReason::BlockAlignmentNotPowerOfTwo.into());
    }
    ...
    if chunk_ratio == 0 {
        return Err(InvalidFormatReason::InvalidChunkRatio.into());
    }
    ...
    if bat_entry_count > format::ABSOLUTE_MAXIMUM_BAT_ENTRY_COUNT {
        return Err(InvalidFormatReason::BatEntryCountTooLarge.into());
    }
    ...
    if bat_length > format::MAXIMUM_BAT_SIZE {
        return Err(InvalidFormatReason::BatSizeTooLarge.into());
    }
    ...
    buf[h1_start..h1_end].copy_from_slice(header.as_bytes());
    let crc = format::compute_checksum(
        &buf[h1_start..h1_start + format::HEADER_SIZE as usize],
        4, // checksum field offset within Header
    );
    buf[h1_start + 4..h1_start + 8].copy_from_slice(&crc.to_le_bytes());
    ...
    buf[h2_start..h2_end].copy_from_slice(header.as_bytes());
    let crc = format::compute_checksum(&buf[h2_start..h2_start + format::HEADER_SIZE as usize], 4);
    buf[h2_start + 4..h2_start + 8].copy_from_slice(&crc.to_le_bytes());
    ...
    buf[rt_start..rt_start + rt_header_bytes.len()].copy_from_slice(rt_header_bytes);
    ...
    buf[entry_offset..entry_offset + bat_entry_bytes.len()].copy_from_slice(bat_entry_bytes);
    ...
    buf[entry_offset2..entry_offset2 + meta_entry_bytes.len()].copy_from_slice(meta_entry_bytes);

    // Compute region table checksum over the full 64 KiB region.
    let rt_end = rt_start + format::REGION_TABLE_SIZE as usize;
    let crc = format::compute_checksum(&buf[rt_start..rt_end], 4);
    buf[rt_start + 4..rt_start + 8].copy_from_slice(&crc.to_le_bytes());

    // Copy region table 1 to region table 2.
    let alt_start = format::ALT_REGION_TABLE_OFFSET as usize;
    let alt_end = alt_start + format::REGION_TABLE_SIZE as usize;
    buf.copy_within(rt_start..rt_end, alt_start);
    debug_assert_eq!(buf[rt_start..rt_end], buf[alt_start..alt_end]);
    ...
    let zero_buf = vec![0u8; log_length as usize];
    file.write_at(log_offset, &zero_buf).await?;

    // --- Build and write the metadata table ---

    let mut meta_buf = vec![0u8; metadata_length as usize];
    ...
    meta_buf[..th_bytes.len()].copy_from_slice(th_bytes);
    ...
    meta_buf[fp_off..fp_off + fp_bytes.len()].copy_from_slice(fp_bytes);

    // Write the virtual disk size item data.
    let ds_off = ds_data_offset as usize;
    meta_buf[ds_off..ds_off + 8].copy_from_slice(&params.disk_size.to_le_bytes());

    // Write the logical sector size item data.
    let lss_off = lss_data_offset as usize;
    meta_buf[lss_off..lss_off + 4].copy_from_slice(&params.logical_sector_size.to_le_bytes());

    // Write the physical sector size item data.
    let pss_off = pss_data_offset as usize;
    meta_buf[pss_off..pss_off + 4].copy_from_slice(&params.physical_sector_size.to_le_bytes());

    // Write the page 83 item data.
    let p83_off = p83_data_offset as usize;
    meta_buf[p83_off..p83_off + 16].copy_from_slice(params.page_83_data.as_bytes());

    // Write the metadata region.
    file.write_at(metadata_offset, &meta_buf).await?;

    // --- Zero the BAT region ---

    let bat_zero = vec![0u8; bat_length as usize];
    file.write_at(bat_offset, &bat_zero).await?;
