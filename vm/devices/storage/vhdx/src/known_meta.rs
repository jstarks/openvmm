...
pub(crate) async fn read_known_metadata(
    ...
    table: &MetadataTable,
    metadata_offset: u64,
) -> Result<KnownMetadata, VhdxError> {
    ...
    let page_83_data = {
        ...
        let data = table
            .read_item(file, metadata_offset, false, &format::PAGE_83_ITEM_GUID)
            .await?;
...
mod tests {
    ...
    async fn verify_unknown_required_item() {
        ...
        assert!(matches!(
            result,
            Err(VhdxError::Corrupt(CorruptionType::UnknownRequiredMetadata))
        ));
    }
    ...
    async fn verify_incomplete_file() {
        ...
        assert!(matches!(
            result,
            Err(VhdxError::Corrupt(CorruptionType::IncompleteFile))
        ));
    ...
    async fn validate_block_size_power_of_two() {
        ...
        let result = read_known_metadata(&file, &table, regions.metadata_offset).await;
        assert!(matches!(
            result,
            Err(VhdxError::Corrupt(CorruptionType::InvalidBlockSize))
        ));
    }
    ...
    async fn validate_sector_sizes() {
        ...
        assert!(matches!(
            result,
            Err(VhdxError::Corrupt(CorruptionType::InvalidLogicalSectorSize))
        ));
    }
}
