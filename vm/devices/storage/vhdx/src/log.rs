...
// This module's items are pub(crate) and will be used by other modules in
// later phases.  Suppress dead-code warnings until then.
#![allow(dead_code)]
...
use crate::format::LOGABLE_OFFSET;
...
impl LogRegion {
    ...
    async fn read_sector<F: AsyncFile>(
        ...
        base: u32,
        offset: u32,
        buf: &mut [u8; SECTOR as usize],
    ) -> Result<(), VhdxError> {
        let pos = self.log_add(base, offset);
        file.read_at(self.file_offset + pos as u64, buf).await?;
        Ok(())
    }

    /// Write `data` to the log at `offset`, handling wrap-around.
    async fn write_at<F: AsyncFile>(
        &self,
        file: &F,
        offset: u32,
        data: &[u8],
        ...
        if remaining as usize >= data.len() {
            // No wrap.
            file.write_at(self.file_offset + offset as u64, data)
                .await?;
        } else {
            // Split at boundary.
            let (first, second) = data.split_at(remaining as usize);
            file.write_at(self.file_offset + offset as u64, first)
                .await?;
            file.write_at(self.file_offset, second).await?;
        }
        Ok(())
    }
}
...
async fn find_log_sequence<F: AsyncFile>(
    ...
    best.ok_or(VhdxError::Corrupt(CorruptionType::NoValidLogEntries))
}
...
async fn expand_sequence<F: AsyncFile>(
    ...
    region
        .read_sector(file, seq.head, 0, &mut sector_buf)
        .await
        .map_err(FindError::Vhdx)?;

    let header = LogEntryHeader::read_from_bytes(&sector_buf[..HEADER_SIZE as usize])
    ...
    // Start with the first sector (header's checksum zeroed).
    let mut crc_buf = sector_buf;
    // Zero checksum field (offset 4, 4 bytes).
    crc_buf[4..8].fill(0);
    let mut crc = crc32c::crc32c(&crc_buf);
    ...
    for i in 0..header.descriptor_count {
        ...
        if byte_offset.is_multiple_of(SECTOR) {
            ...
            region
                .read_sector(file, seq.head, byte_offset, &mut sector_buf)
                .await
                .map_err(FindError::Vhdx)?;
            crc = crc32c::crc32c_append(crc, &sector_buf);
        }
    ...
    for i in 0..data_descriptor_count {
        ...
        region
            .read_sector(file, seq.head, offset, &mut sector_buf)
            .await
            .map_err(FindError::Vhdx)?;
        crc = crc32c::crc32c_append(crc, &sector_buf);

        let data_sector =
            LogDataSector::read_from_bytes(&sector_buf).map_err(|_| FindError::NoSequence)?;
...
fn validate_data_descriptor(
    ...
    if desc.file_offset < LOGABLE_OFFSET {
        return false;
    }
...
async fn apply_sequence<F: AsyncFile>(
    ...
    while tail != head {
        // Read header.
        region.read_sector(file, tail, 0, &mut sector_buf).await?;
        let header = LogEntryHeader::read_from_bytes(&sector_buf[..HEADER_SIZE as usize])
        ...
        for i in 0..header.descriptor_count {
                ...
                if byte_offset >= SECTOR {
                    region
                        .read_sector(
                            file,
                            tail,
                            byte_offset - (byte_offset % SECTOR),
                            &mut sector_buf,
                        )
                        .await?;
                }
            ...
            if sig == LOG_DESCRIPTOR_ZERO_SIGNATURE {
                ...
                file.write_at(desc.file_offset, &zeros).await?;
                ...
                let mut data_buf = [0u8; SECTOR as usize];
                region
                    .read_sector(file, tail, data_offset, &mut data_buf)
                    .await?;
                ...
                data_buf[0..8].copy_from_slice(&leading);
                // Replace last 4 bytes (sequence_low) with trailing_bytes.
                let trailing = desc.trailing_bytes.to_le_bytes();
                data_buf[SECTOR as usize - 4..].copy_from_slice(&trailing);

                file.write_at(desc.file_offset, &data_buf).await?;
                data_sector_index += 1;
            } else {
                return Err(VhdxError::Corrupt(CorruptionType::BadLogEntryOnReplay));
            }
    ...
    if replayed {
        let file_sz = file.file_size().await?;
        if file_sz < last_file_offset {
            file.set_file_size(last_file_offset).await?;
        }
        file.flush().await?;
    }
...
impl LogWriter {
    ...
    ///
    /// Fails if the log doesn't have enough free space.
    pub async fn write_entry<F: AsyncFile>(
        ...
        if self.tail == self.head {
            ...
            if needed > self.region.length {
                return Err(VhdxError::Corrupt(CorruptionType::LogFull));
            }
        ...
        let mut buf = vec![0u8; elen as usize];
        ...
        for dp in data_pages {
            ...
            let leading = u64::from_le_bytes(dp.data[0..8].try_into().unwrap());
            let trailing = u32::from_le_bytes(
                dp.data[SECTOR as usize - 4..SECTOR as usize]
                    .try_into()
                    .unwrap(),
            );
            ...
            buf[desc_offset..desc_offset + DESCRIPTOR_SIZE as usize]
                .copy_from_slice(desc.as_bytes());
            desc_offset += DESCRIPTOR_SIZE as usize;

            // Build data sector.
            let ds = build_data_sector(dp.data, seq);
            buf[data_sector_offset..data_sector_offset + SECTOR as usize]
        ...
        // Compute CRC-32C (with checksum field zeroed — it already is).
        let crc = crc32c::crc32c(&buf);
        // Write checksum into the buffer.
        buf[4..8].copy_from_slice(&crc.to_le_bytes());

        // Write the entry to the log region.
        self.region.write_at(file, self.head, &buf).await?;
...
mod tests {
    ...
    async fn writer_initialize_creates_empty_entry() {
        ...
        let mut tmp = buf;
        tmp[4..8].fill(0);
        let computed_crc = crc32c::crc32c(&tmp);
    ...
    async fn writer_one_data_page() {
        ...
        let target_offset = LOGABLE_OFFSET + 4096;
    ...
    async fn writer_one_zero_range() {
        ...
        let target_offset = LOGABLE_OFFSET;
        let lsn = writer
            .write_entry(
                &file,
                &[],
    ...
    async fn writer_log_full_error() {
        ...
        match result.unwrap_err() {
            VhdxError::Corrupt(CorruptionType::LogFull) => {}
    ...
    async fn replay_data_and_zero_descriptors() {
        ...
        let zero_target = LOGABLE_OFFSET + 8192;
        ...
        page_data[0] = 0xDE;
        page_data[1] = 0xAD;
        let data_target = LOGABLE_OFFSET + 4096;
    ...
    async fn replay_multiple_sequential_entries() {
        ...
        for (n, page) in pages.iter().enumerate() {
            let offset = LOGABLE_OFFSET + (n as u64) * 4096;
            writer
                .write_entry(
                    &file,
                    &[DataPage {
    ...
    async fn replay_invalid_crc_skipped() {
        ...
        let mut read_buf = [0u8; SECTOR as usize];
        file.read_at(LOGABLE_OFFSET, &mut read_buf).await.unwrap();
    ...
    async fn replay_wrong_guid_skipped() {
        ...
        assert!(result.is_err());
        match result.unwrap_err() {
            VhdxError::Corrupt(CorruptionType::NoValidLogEntries) => {}
    ...
    async fn replay_torn_write_last_entry() {
        ...
        let mut buf = [0u8; SECTOR as usize];
        file.read_at(LOGABLE_OFFSET, &mut buf).await.unwrap();
        assert_eq!(buf, page1);
    ...
    async fn writer_then_replay_roundtrip() {
        ...
        for n in 0..5 {
            ...
            pages.push((LOGABLE_OFFSET + (n as u64) * 4096, page));
        }
    ...
    async fn replay_entry_wrapping_circular_buffer() {
        ...
        writer
            .write_entry(
                &file,
                &[DataPage {
                    file_offset: LOGABLE_OFFSET + 12288,
                    data: &page_wrap2,
                }],
                &[],
            )
        ...
        file.read_at(LOGABLE_OFFSET + 12288, &mut buf)
            .await
            .unwrap();
    ...
    async fn roundtrip_crash_replay() {
        ...
        let target = LOGABLE_OFFSET + 4096;
        writer
            .write_entry(
                &file,
                &[DataPage {
