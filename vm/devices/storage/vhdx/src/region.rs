...
pub(crate) struct ParsedRegions {
...
async fn read_and_validate_region_table(
    file: &impl AsyncFile,
    offset: u64,
) -> Result<Option<Vec<u8>>, VhdxError> {
    let mut buf = vec![0u8; format::REGION_TABLE_SIZE as usize];
    file.read_at(offset, &mut buf).await?;

    // Check signature.
    let header = match RegionTableHeader::read_from_prefix(&buf) {
        Ok((h, _)) => h,
...
pub(crate) async fn parse_region_tables(
    ...
    let (table_buf, needs_rewrite) = match (&table1, &table2) {
        (Some(t1), Some(t2)) => {
            let needs_rewrite = t1 != t2;
            (t1, needs_rewrite)
        }
    ...
    let header = RegionTableHeader::read_from_prefix(table_buf)
        .unwrap()
        .0
        .clone();
    ...
    for i in 1..by_offset.len() {
        let prev_end = by_offset[i - 1].0 + by_offset[i - 1].1;
...
mod tests {
    ...
    async fn parse_both_corrupt() {
        ...
        assert!(matches!(
            result,
            Err(VhdxError::Corrupt(CorruptionType::RegionTablesBothCorrupt))
        ));
    }
    ...
    async fn parse_duplicate_region() {
        ...
        assert!(matches!(
            result,
            Err(VhdxError::Corrupt(CorruptionType::DuplicateRegionEntry))
        ));
    }
