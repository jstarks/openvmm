...
mod tests {
    ...
    /// first bitmap page.
    ///
    /// Returns `(file, VhdxFile, data_block_file_offset, sbm_block_file_offset)`.
    async fn create_partial_block_vhdx(
        bitmap_data: &[u8; 4096],
    ) -> (InMemoryFile, VhdxFile<InMemoryFile>, u64, u64) {
        ...
        // Return the data/sbm offsets for test assertions.
        (
            // We need the actual file back - re-create everything
            // Actually, create a second InMemoryFile. Better approach:
            // just return the VhdxFile and the offsets.
            {
                // We can't get the file back from VhdxFile, so create a
                // duplicate setup. Actually, let's just return the VhdxFile.
                // For assertions we only need offsets.
                // The file is moved into VhdxFile, so we can't return it.
                // Instead, just provide a dummy.
                InMemoryFile::new(0) // placeholder, tests use VhdxFile
            },
            vhdx,
            data_block_offset,
            sbm_block_offset,
    ...
    async fn partial_block_all_transparent() {
        ...
        let (_, vhdx, _, _) = create_partial_block_vhdx(&bitmap).await;
