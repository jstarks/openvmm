...
use crate::ExportedMemoryRegion;
...
impl AsyncResolveResource<VirtioDeviceHandle, VhostUserDeviceHandle> for VhostUserFrontendResolver {
    ...
    async fn resolve(
        ...
        // Get shareable memory regions for SET_MEM_TABLE.
        let sharing = input.guest_memory.guest_memory_sharing_control().context(
            "guest memory is not shareable \u{2014} \
                 vhost-user requires shared (memfd-backed) guest RAM",
        )?;

        let exported_regions: Vec<ExportedMemoryRegion> = sharing
            .regions()
            .into_iter()
            .map(|r: ShareableRegion| ExportedMemoryRegion {
                guest_phys_addr: r.guest_address,
                size: r.size,
                fd: r.file,
                mmap_offset: r.file_offset,
            })
            .collect();
