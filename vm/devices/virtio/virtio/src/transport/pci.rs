...
/// categorize the device correctly. Values follow QEMU's per-device choices.
...
impl VirtioPciDevice {
    ...
    fn read_pci_cfg_bar(&mut self, bar: u8, offset: u32, data: &mut [u8]) -> IoResult {
        match bar {
            ...
            _ => {
                tracelimit::warn_ratelimited!(bar, offset, "pci_cfg_data read to unsupported bar");
    ...
    fn write_pci_cfg_bar(&mut self, bar: u8, offset: u32, data: &[u8]) -> IoResult {
        match bar {
            ...
            _ => {
                tracelimit::warn_ratelimited!(bar, offset, "pci_cfg_data write to unsupported bar");
                return IoResult::Err(IoError::InvalidRegister);
            }
        }
        IoResult::Ok
