...
fn tx_offload_no_offloads() {
    ...
    let meta = Worker::parse_tx_offloads(Some(&header), &[], 1000);
    assert_eq!(
...
fn tx_offload_none_header() {
    let meta = Worker::parse_tx_offloads(None, &[], 500);
...
fn tx_offload_tcp_checksum() {
    ...
    let meta = Worker::parse_tx_offloads(Some(&header), &make_eth_header(ETHERTYPE_IPV4), 1000);
...
fn tx_offload_udp_checksum() {
    ...
    let meta = Worker::parse_tx_offloads(Some(&header), &make_eth_header(ETHERTYPE_IPV4), 800);
...
fn tx_offload_ipv6_tcp_checksum() {
    ...
    let meta = Worker::parse_tx_offloads(Some(&header), &make_eth_header(ETHERTYPE_IPV6), 1000);
    assert!(meta.flags.offload_tcp_checksum());
...
fn tx_offload_tso4() {
    ...
    let meta = Worker::parse_tx_offloads(Some(&header), &make_eth_header(ETHERTYPE_IPV4), 64000);
...
fn tx_offload_tso6() {
    ...
    let meta = Worker::parse_tx_offloads(Some(&header), &make_eth_header(ETHERTYPE_IPV6), 64000);
...
async fn tx_offload_end_to_end_tcp_csum(driver: DefaultDriver) {
    ...
    // Verify the logged segments contain offload metadata
    let log = handle.take_tx_avail_log();
    assert_eq!(log.len(), 1);
    assert_eq!(log[0].len(), 1);
    assert!(log[0][0].is_head);
}
