...
impl SynicClient for HclSynic {
    ...
    fn map_event(&self, event_flag: u16, event: &pal_event::Event) -> io::Result<()> {
        self.hcl_vmbus
            .set_eventfd(event_flag as u32, Some(event.as_fd()))
            .map_err(|err| io::Error::new(io::ErrorKind::Other, err))
    }

    fn unmap_event(&self, event_flag: u16) {
        self.hcl_vmbus.set_eventfd(event_flag as u32, None).unwrap();
    }

    fn signal_event(&self, connection_id: u32) -> io::Result<()> {
        self.hcl_vmbus
            .signal_event(connection_id, 0)
            .map_err(|err| io::Error::new(io::ErrorKind::Other, err))
    }
}
