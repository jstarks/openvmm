...
pub struct Aarch64PlatformConfig {
    ...
    /// GIC MSI delivery configuration for PCIe interrupts.
    pub gic_msi: Aarch64GicMsiConfig,
...
/// Construction-time GIC MSI configuration.
///
/// Carries only the two orthogonal pieces of information that must be baked
/// into partition construction: the physical passthrough MSI doorbell address
/// and whether the GIC exposes LPI support. The concrete MSI-controller
/// instance data (v2m frame details, per-segment ITS MMIO bases) is *not*
/// part of the topology — it is assigned after `build()` by the memory-layout
/// allocator and lives with the device-land MSI controller objects and the
/// ACPI/DT description.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[cfg_attr(feature = "inspect", derive(inspect::Inspect))]
pub struct Aarch64GicMsiConfig {
    /// Physical base of the passthrough MSI doorbell. The hypervisor traps
    /// assigned-device MSI writes here and injects them (mshv registers it as
    /// `GitsTranslaterBaseAddress`; WHP uses it as the physical remap base).
    /// Always present, independent of LPI support.
    #[cfg_attr(feature = "inspect", inspect(hex))]
    pub passthrough_msi_doorbell: u64,
    /// Whether the GIC exposes LPI support (`GICD_TYPER.LPIS`). `false` for
    /// v2m/SPI delivery, `true` for ITS.
    pub lpi_enabled: bool,
}
...
impl ProcessorTopology<Aarch64Topology> {
    ...
    pub fn gic_msi(&self) -> Aarch64GicMsiConfig {
