...
/// and assign a VFIO device from the hosting VM into the guest.
///
/// This test is intended to run inside a QEMU TCG hosting VM with KVM.
/// The hosting VM profile sets up a virtio-blk device bound to vfio-pci,
/// and exports its BDF via the `HOSTING_VM_VFIO_BDF_TEST_DISK` env var.
